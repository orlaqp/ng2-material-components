...
export class SelectPickerComponent extends InputBase implements OnInit, AfterViewInit {
    ...
    @Input() noneResultsText: string = 'No results matched {0}';
    ...
    @Input() liveSearchStyle: string = 'contains';
    ...
    ngAfterViewInit() {
        this.$element = $(this._el.nativeElement).find('.my-selectpicker');
  ...
  /**
   ...
   private liHeight(refresh?: boolean) {
     ...
     */
    private setSelected(index, selected, $lis) {
      if (!$lis) {
     ...
     * @param {JQuery} $lis - the 'li' element that is being modified
     */
    private setDisabled(index, disabled, $lis) {
      if (!$lis) {
        $lis = this.findLis().eq(this.liObj[index]);
      }
